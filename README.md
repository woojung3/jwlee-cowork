...
- LiteLLM/Vertex AI 연동 시 `LITELLM_MASTER_KEY` 또는 `LITELLM_API_KEY` 환경 변수 설정 필요

### 실행 방법
Spring Shell과의 아규먼트 충돌 방지를 위해, 프로파일 지정 시 항상 **시스템 프로퍼티(`-Dspring-boot.run.profiles`)** 또는 **환경 변수(`SPRING_PROFILES_ACTIVE`)** 방식을 사용함
...
#### 3. Vertex AI / LiteLLM 프로파일 실행
```bash
# API 키 환경 변수 설정
export LITELLM_MASTER_KEY="your-master-key"

# 환경 변수 방식으로 실행
SPRING_PROFILES_ACTIVE=vertex ./mvnw spring-boot:run
```

실행 후 `>` 프롬프트가 나타나면 명령어 입력을 대기함

### 3. 비대화형(Non-interactive) 모드 실행 (자동화)
에이전트 명령어를 자동화 도구(Cron 등)에 등록할 때 사용하며, `cron` 프로파일(`application-cron.yml`)을 활성화하면 셸 진입 없이 명령을 즉시 수행하고 종료함
...
- **실행 방법**:
  ...
  ```
- **전용 스크립트 활용**: `scripts/jwlee-cowork.sh`를 사용하면 환경 변수(`SPRING_PROFILES_INCLUDE=cron`) 설정을 통해 더 간편하게 자동화를 구현함
